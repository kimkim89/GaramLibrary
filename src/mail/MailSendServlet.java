...
public class MailSendServlet extends HttpServlet {
			...
			HttpServletResponse response) throws SecurityException, IOException
	{
		...
		try {
			...
			Address sender_address = new InternetAddress("ansqkdtkdn1@gmail.com");
			Address receiver_address = new InternetAddress("ansqkdtkdn1@gmail.com");
			message.setHeader("content-type", "text/html;charset=UTF-8");
			message.setFrom(sender_address);
			message.addRecipient(Message.RecipientType.TO, receiver_address);
			message.setSubject(subject);
			message.setContent(in, "text/html;charset=UTF-8");
			...
			out.println("<script>"
					+ " alert('메일이 전송되었습니다.');"
					+ " </script>");
			response.sendRedirect(pagecontext+"/index.jsp");
